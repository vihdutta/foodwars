import { background_init, menu_dimmer_init, player_init, coordinates_text_init, fps_text_init, inventory_init, health_bar_init, health_bar_value_init, socket_text_init, notification_init, bullet_count_init, ping_init, wall_count_init, centering_test_init } from './graphics.js';
...
setInterval(() => {
    if (playing) {
        socket.emit("serverUpdateSelf", {
            id: socket.id,
            username: username,
            rotation: Math.atan2(mouse.y - app.renderer.height / 2, mouse.x - app.renderer.width / 2) + Math.PI / 2,
            mb1: mouse.mb1,
            keyboard: keyboard
        });
    }
...
app.ticker.add(() => {
    updateCamera(app, player, widthForHealthBar, camera, UIElements, dimRectangle, coordinatesText, FPSText, socketText, inventory, healthBar, healthBarValue, notificationContainer, bulletCount, pingText, wallCount);
});
...
// PING
socket.on("pong", (serverTime, clientTime) => {
    const now = Date.now();
...
function layoutUI() {
    ...
    const pad = 0.02;
    centering_test.position.set(w * pad, h * (1 - pad) - centering_test.height);
}
layoutUI();
